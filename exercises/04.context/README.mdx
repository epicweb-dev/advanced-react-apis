...
<callout-warning>
	The `use` hook we're using here is not officially released in React but will
	soon be. For now, in a regular React app on `<= 18.2`, you'll use `useContext`.
	In the workshop we're using an experimental version of React that has this
	feature.
</callout-warning>

{/* prettier-ignore */}
<callout-warning>
Soon you'll be able to render `<CountContext />` rather than
