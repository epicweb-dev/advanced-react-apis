...
First argument in useReducer is a the reducer function. The second is the
initial state value.

const [state, dispatch] = useReducer(reducerFunction, intialState)

function reducer(state, newState) { return new state }

whatever is passed to the dipatch, will be the second argument passed to the
reducer function. this is referred to as the action.

the first argument to the second argumne to our reducer function is the
dispatch.
