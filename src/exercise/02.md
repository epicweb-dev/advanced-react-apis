...
Performance optimization technique (certain form of caching)

Principle:

- store the original computation for a given input
  - if a **new input** is given, **recompute & store**
  - if the **same input** is given, **return stored computation**

In React, there's two hooks for memoization:
...
There's two things we need memoization for:

1. Referential equality
2. Computationally expensive calculations

Refer to the whole article
[here](https://kentcdodds.com/blog/usememo-and-usecallback).
