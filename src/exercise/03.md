...
`context` allows us to make a **state available to a section** of our react
tree, without having to pass it down via props.

To use `context`:

1. **create a context** (outside of the components). `createContext` accepts a
   default value as a first argument, however, usually you would want
   `createContext` to return `undefined` in case it is used outside of a
   provider, and not fall back to a certain value.
   ```js
   const MyContext = React.createContext()
   ```
2. **wrap the desired section in a provider**
   ```js
   ReactDOM.render(
     <MyContext.Provider value="so cool!">
       <MyComponent />
     </MyContext.Provider>,
     document.getElementById('root'),
   )
   ```
3. **retrieve the context's state** within a component

   ```js
   const MyComponent = () => {
     const myContext = React.useContext(MyContext)

     return <p>This is {myContext}</p>
     // renders: This is so cool!
   }
   ```

### Custom context hook

Extracting the context into a custom hook allows proper error handling (e.g. in
case the context is used outside of a provider), which clusters logic and
prevents misusage.
...
const useMyContext = () => {
  ...
  if (!context) {
    throw new Error('useMyContext can only be used within MyContextProvider')
  }
