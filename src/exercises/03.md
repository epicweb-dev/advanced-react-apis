...
The article should illustrate the problem with having a `value` that's
initialized every render. To solve that problem, we're going to make two
contexts. One for the `count` and the other for `setCount`. We can render them
both in the `CountProvider`, and then we just consume the context we want when
we want it.
